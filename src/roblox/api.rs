...
///
/// Returns `1s`, `2s`, `4s` for attempts 1, 2, 3 respectively,
/// capped by available `MAX_RETRIES`.
...
async fn get_json<T: serde::de::DeserializeOwned>(url: &str) -> Result<T, ApiError> {
    ...
    for attempt in 0..MAX_RETRIES {
        if attempt > 0 {
            // Respect Retry-After from a 429 if we saw one, otherwise exponential backoff.
            let delay = match &last_err {
                Some(ApiError::RateLimited { retry_after }) => {
                    Duration::from_secs(retry_after.unwrap_or(backoff_delay(attempt).as_secs()))
                }
